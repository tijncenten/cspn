...
def get_data(name):
    if 'redwine' in name:
        data_red = pd.read_csv(f'{dataset_path}/winequality_red.csv')
        data = pd.concat([data_red]).values
        data[:, -1] = np.where(data[:, -1] <= 6, 0, 1)
        ncat = learncats(data, classcol=data.shape[1]-1)
    elif 'whitewine' in name:
        data_white = pd.read_csv(f'{dataset_path}/winequality_white.csv')
        data = pd.concat([data_white]).values
        data[:, -1] = np.where(data[:, -1] <= 6, 0, 1)
        ncat = learncats(data, classcol=data.shape[1]-1)
    elif 'wine' in name:
        data_red = pd.read_csv(f'{dataset_path}/winequality_red.csv')
        ...
        data = pd.read_csv(f'{dataset_path}/bank-additional-full.csv', sep=';')
        data, ncat = bank(data)
    elif 'segment' in name:
        data = pd.read_csv(f'{dataset_path}/segment.csv')
        data, ncat = segment(data)
    elif 'german' in name:
        data = pd.read_csv(f'{dataset_path}/german.csv', sep=' ', header=None)
        data, ncat = german(data)
    elif 'vehicle' in name:
        data = pd.read_csv(f'{dataset_path}/vehicle.csv')
        data['Class'] = get_dummies(data['Class'])
        ncat = np.ones(data.shape[1])
        ncat[-1] = len(np.unique(data['Class']))
        data = data.values.astype(float)
    elif 'vowel' in name:
        data = pd.read_csv(f'{dataset_path}/vowel.csv')
        data, ncat = vowel(data)
    elif 'authent' in name:
        data = pd.read_csv(f'{dataset_path}/authent.csv')
        data['Class'] = get_dummies(data['Class'])
        ncat = learncats(data.values).astype(int)
        data = data.values.astype(float)
    elif 'diabetes' in name:
        data = pd.read_csv(f'{dataset_path}/diabetes.csv')
        data['class'] = get_dummies(data['class'])
        ...
        data = pd.read_csv(f'{dataset_path}/cmc.csv')
        data, ncat = cmc(data)
    elif 'electricity' in name:
        data = pd.read_csv(f'{dataset_path}/electricity.csv')
        data, ncat = electricity(data)
    elif 'gesture' in name:
        data = pd.read_csv(f'{dataset_path}/gesture.csv')
        data['Phase'] = get_dummies(data['Phase'])
        data = data.values.astype(float)
        ncat = np.ones(data.shape[1])
        ncat[-1] = 5
    elif 'breast' in name:
        data = pd.read_csv(f'{dataset_path}/wdbc.csv')
        data['Class'] = get_dummies(data['Class'])
        data = data.values.astype(float)
        ncat = np.ones(data.shape[1])
        ncat[-1] = 2
    elif 'krvskp' in name:
        data = pd.read_csv(f'{dataset_path}/kr-vs-kp.csv')
        data = get_dummies(data)
        ncat = learncats(data.values)
        data = data.values.astype(float)
    elif 'dna' in name:
        data = pd.read_csv(f'{dataset_path}/dna.csv')
        data = get_dummies(data).values.astype(float)
        ncat = learncats(data)
    elif 'robot' in name:
        data = pd.read_csv(f'{dataset_path}/robot.csv')
        data['Class'] = get_dummies(data['Class'])
        data = data.values.astype(float)
        ncat = learncats(data)
    elif 'mice' in name:
        data = pd.read_csv(f'{dataset_path}/miceprotein.csv')
        data['class'] = get_dummies(data['class'])
        data = data.replace('?', np.nan)
        ...
        data = pd.read_csv(f'{dataset_path}/dresses.csv')
        data = data.replace('?', np.nan)
        data = get_dummies(data)
        data = data.values.astype(float)
        data[data < 0] = np.nan
        ...
        data = pd.read_csv(f'{dataset_path}/texture.csv')
        data['Class'] = get_dummies(data['Class'])
        data = data.values.astype(float)
        ncat = np.ones(data.shape[1])
        ncat[-1] = 11
    elif 'splice' in name:
        data = pd.read_csv(f'{dataset_path}/splice.csv')
        data = data.drop('Instance_name', axis=1)
        data = get_dummies(data).values.astype(float)
        ncat = learncats(data)
    elif 'jungle' in name:
        data = pd.read_csv(f'{dataset_path}/jungle.csv')
        data = get_dummies(data)
        data = data.values.astype(float)
        ncat = learncats(data)
    elif 'phishing' in name:
        data = pd.read_csv(f'{dataset_path}/phishing.csv')
        data = get_dummies(data)
        data = data.values.astype(float)
        ncat = learncats(data)
    elif 'fashion' in name:
        data = pd.read_csv(f'{dataset_path}/fashion.csv')
        data = data.values.astype(np.float64)
        ncat = np.ones(data.shape[1]).astype(np.int64)
        ncat[-1] = 10
    elif 'mnist' in name:
        data = pd.read_csv(f'{dataset_path}/mnist.csv')
        data = data.values.astype(np.float64)
        ncat = np.ones(data.shape[1]).astype(np.int64)
        ncat[-1] = 10
    else:
